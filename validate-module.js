  ...
  for (const simulation of SIMULATIONS) {
    try {
      ...
      if (response.status === 429) {
        console.log(`⚠️ Rate limited on case ${simulation.id}. Waiting extra time...`);
        await sleep(30000); // Wait 30 seconds if limited
      } else if (data.riskAssessment) {
        console.log(`✅ Case ${simulation.id} (${simulation.emotion}) passed: Risk level ${data.riskAssessment.riskLevel}`);
      } else {
        console.log(`❌ Case ${simulation.id} failed: Malformed response.`);
      }
    } catch (e) {
      console.log(`❌ Connection error on case ${simulation.id}`);
    }
    
    // Standard wait between tests
    console.log("⏳ Pausing...");
    await sleep(25000); 
  }
