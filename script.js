...
const API_BASE = 'http://localhost:3003'; // same-origin; change to e.g. 'http://localhost:3003' if frontend is served separately
...
fetch('http://localhost:3003/api/chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ messages: [{ role: 'user', content: yourMessage }] })
})

// At the bottom of script.js
fetch('http://localhost:3003/api/chat')
// This code is dangling at the end of the file with no function scope